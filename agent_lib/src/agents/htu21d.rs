use rppal::i2c::{I2c, Error as RPPALError};
use std::fmt;
use crate::error;
...
impl HTU32D {
    pub fn new(temperature_unit: TemperatureUnits) -> Self {
        ...
        Self {comm_channel, temperature_unit}
    }
    ...
    fn validate(data: &[u8], expected: u8) ->Result<(), crate::error::Error> {
        ...
        if checksum == expected {
            ...
            Err(error::Error::ChecksumError { expected: checksum as u32, actual: expected as u32, data: data[0..2].to_vec()})
        }
    }

    pub fn read_temperature(self: &mut Self) -> Result<Temperature, error::BoxAgentLibError> {
        self.comm_channel.write(&[TEMPERATURE_REGISTER_ADDRESS as u8])?;
    ...
    pub fn read_humidity(self: &mut Self) -> Result<Humidity, error::BoxAgentLibError> {
        self.comm_channel.write(&[HUMIDITY_REGISTER_ADDRESS as u8])?;
    ...
    fn calculate_temperature(self: &Self, data: &[u8]) -> Result<Temperature, error::BoxAgentLibError> {
        let (signal_value, checksum_value) = self.parse_data_buffer(&data);
        Self::validate(&data, checksum_value)?;
        ...
        Ok(Temperature { temperature: converted_value, unit:  self.temperature_unit}) 
    }
...
mod tests{
   ...
   #[test]
    ...
    fn signal_to_temperature_test_1() {
        ...
        assert!((temp - expected).abs() <= TEMPERATURE_EPSILON,
            "Expected {} but got {}", expected, temp);
    }
    ...
    fn signal_to_humdity_test_1() {
        ...
        assert!((relative_humidity - expected).abs() <= RELATIVE_HUMIDITY_EPSILON,
            "Expected {} but got {}", expected, relative_humidity);
    }
